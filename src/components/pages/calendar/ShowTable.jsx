...
const ShowTable = (props) => {
  ...
  const getColor = (status) => {
    if (status === 0) {
      return (
        <TableCell align="right" sx={{ backgroundColor: 'pink' }}>
          {getStatus(status)}
        </TableCell>
      );
    } else if (status === 1) {
      return (
        <TableCell align="right" sx={{ backgroundColor: 'yellow' }}>
          {getStatus(status)}
        </TableCell>
      );
    } else if (status === 2) {
      return (
        <TableCell align="right" sx={{ backgroundColor: 'green', color: 'white' }}>
          {getStatus(status)}
        </TableCell>
      );
    } else if (status === 3) {
